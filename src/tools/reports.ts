...
export function registerReportTools(
  ...
  server.registerTool(
    ...
    async () => {
      const result = await client.get("advanced-reports/");
  ...
  server.registerTool(
    ...
    async ({ report_id }) => {
      const result = await client.get(`advanced-reports/${report_id}/`);
  ...
  server.registerTool(
    ...
    async ({ report_id, format }) => {
      ...
      const result = await client.get(
        `advanced-reports/${report_id}/run/`,
        params
      );
  ...
  server.registerTool(
    ...
    async ({ title, chart, query, description, chart_meta, type }) => {
      ...
      const result = await client.apiRequest("POST", "advanced-reports/", data);
          ...
          content: [
            {
              ...
              text: JSON.stringify(
                {
                  error:
                    "Query must include at least one of 'dimensions' or 'measures'",
                },
                null,
                2
              ),
